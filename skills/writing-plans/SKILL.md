...
**优先读取顺序（新格式）：**
1. `docs/superpowers/specs/YYYY-MM-DD-<topic>-spec.md` — 需求规格（WHAT），包含 WHEN/THEN 场景
2. `docs/superpowers/specs/YYYY-MM-DD-<topic>-design.md` — 技术设计（HOW），包含架构和决策
...
**1. 规格覆盖度：** 浏览 SPEC.md 中的每个需求和 WHEN/THEN 场景（如果有 SPEC.md）。你能指出实现它的任务吗？检查每个 WHEN/THEN 场景是否在计划中有对应的测试或验证步骤。列出所有遗漏。
